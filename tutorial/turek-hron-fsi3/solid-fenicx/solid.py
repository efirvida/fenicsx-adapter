...
FROM_X, TO_X = 0.25, 0.6
FROM_Y, TO_Y = 0.19, 0.21
FROM_Z, TO_Z = -0.1, 0.1
...
domain = create_rectangle(
    MPI_COMM,
    [np.array([FROM_X, FROM_Y]), np.array([TO_X, TO_Y])],
    [NX, NY],
...
def clamped_boundary(x):
    return np.isclose(x[0], FROM_X)


def neumann_boundary(x):
    X = np.abs(x[0] - TO_X) < tol
    Y1 = np.abs(x[1] - FROM_Y) < tol
    Y2 = np.abs(x[1] - TO_Y) < tol
    Z1 = np.abs(x[2] - FROM_Z) < tol
    Z2 = np.abs(x[2] - TO_Z) < tol
    return np.logical_or.reduce([X, Y1, Y2, Z1, Z2])
